...
CARTPOLE_FINITE_HORIZON_TIME: int=200


class CustomCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    f"""
    Based on the gymnasium CartPoleEnv

    Changes:
        * Fixed {CARTPOLE_FINITE_HORIZON_TIME=} horizon
        * Reward is sinusoid of pole angle (much better shaped)
        * No terminating states
    """
    ...
    def step(self, action):
        ...
        terminated = self.episode_steps >= CARTPOLE_FINITE_HORIZON_TIME

        if self.render_mode == "human":
            self.render()

        return np.array(self.state, dtype=np.float32), costheta, terminated, False, {}
...
class CustomCartPoleVectorEnv(VectorEnv):
    metadata = {
        "render_modes": ["human", "rgb_array"],
        "render_fps": 50,
    }
    ...
    def step(
        ...
        # Always false
        terminated = self.episode_steps >= CARTPOLE_FINITE_HORIZON_TIME
        truncated = self.total_steps >= self.max_episode_steps
        ...
        if any(done):
            ...
            self.total_steps[done] = 0

        if self.render_mode == "human":
            self.render()

        return self.state.T, costheta, terminated, truncated, {}
...
def register_custom_envs():
    gym.register(
        ...
        entry_point="reinforcement.custom_envs.cartpole:CustomCartPoleEnv",
        vector_entry_point="reinforcement.custom_envs.cartpole:CustomCartPoleVectorEnv",
        max_episode_steps=CARTPOLE_FINITE_HORIZON_TIME,
